...
function UploadStep({ active }) {
  ...
  return (
    <div className="space-y-4">
      {/* Drop zone */}
      <div
        className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all duration-500 ${
      ...
      <div className="flex gap-2">
        <div className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 flex items-center gap-2">
          ...
          <span
            className={`text-xs transition-all duration-500 ${active ? "text-white/60" : "text-white/20"}`}
          >
      ...
      <div className="flex gap-2 justify-center">
        {["YouTube", "TikTok", "Instagram", "Facebook"].map((p) => (
          <span
            key={p}
            className={`text-[10px] px-2 py-1 rounded-md border transition-all duration-300 ${
              active
  ...
  return (
    <div className="bg-off-black rounded-2xl border border-white/[0.07] overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/[0.06]">
        <div
          className={`w-1.5 h-1.5 rounded-full transition-all ${active ? "bg-accent2 animate-pulse" : "bg-white/20"}`}
        />
        ...
        {active && revealed < lines.length && (
          <div className="flex gap-1 pt-1">
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="w-1 h-1 rounded-full bg-accent2/40 animate-pulse"
  ...
  return (
    <div className="space-y-3">
      {CLIPS.map((clip, i) => (
        <div
          key={clip.title}
          className={`bg-off-black border rounded-xl p-3.5 flex items-center gap-3 transition-all duration-500 ${
            active && scores[i] > 0
        ...
        >
          ...
          <div className="text-right flex-shrink-0">
            <p className="text-sm font-display font-bold text-accent2">
              {scores[i]}%
            </p>
      ...
      <div className="relative">
        ...
        {active && (
          <div className="absolute -right-16 top-1/3 bg-card border border-white/[0.07] rounded-xl px-2.5 py-2 animate-float">
            ...
            <p className="text-xs font-display font-bold text-accent2">
              TikTok
            </p>
          </div>
        )}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        {voices.map((v, i) => (
          ...
          >
            <div
              className={`w-8 h-8 rounded-full mx-auto mb-1.5 flex items-center justify-center text-xs font-bold font-display ${v.color}`}
            >
              {v.name[0]}
            </div>
            <p className="text-[10px] font-medium text-white">{v.name}</p>
            <p className="text-[9px] text-white/30 leading-tight mt-0.5">
              {v.tag}
            </p>
          </div>
        ))}
      ...
      {/* Waveform player */}
      <div
        className={`bg-off-black border rounded-2xl p-4 transition-all duration-300 ${
...
function ExportStep({ active }) {
  ...
  useEffect(() => {
    if (!active) {
      setDownloaded([false, false, false, false]);
      return;
    }
    CLIPS.forEach((_, i) => {
      setTimeout(
  ...
  return (
    <div className="space-y-2.5">
      {CLIPS.map((clip, i) => (
        <div
          key={clip.title}
          className={`flex items-center gap-3 bg-off-black border rounded-xl px-4 py-3 transition-all duration-500 ${
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p
              className={`text-[9px] font-medium mt-0.5 ${platforms[i].color}`}
            >
      ...
      {active && downloaded[3] && (
        <div className="bg-accent2/5 border border-accent2/15 rounded-xl p-3 text-center mt-2">
          <p className="text-xs font-medium text-accent2 mb-0.5">
            🎉 All clips exported!
          </p>
          <p className="text-[10px] text-white/35">
            4 clips · 1080p · Ready to post
          </p>
        </div>
      )}
    </div>
      ...
      <div className="max-w-6xl mx-auto px-6 pt-28 pb-20">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-white/40 text-lg font-light max-w-xl mx-auto">
            Watch how VIRA turns a 47-minute video into 4 viral clips —
            automatically.
          </p>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-12">
          {[
            ...
            <div
              key={label}
              className="bg-card border border-white/[0.07] rounded-2xl p-4 text-center"
            >
              <p className="font-display font-bold text-2xl text-white">
                {activeStep >= 2 ? (
                  <Counter to={parseInt(value)} suffix={suffix} />
                ) : (
                  `0${suffix}`
                )}
              </p>
              <p className="text-[10px] text-white/35 mt-1 uppercase tracking-widest font-medium">
                {label}
              </p>
            </div>
          ))}
            ...
            {STEPS.map((step, i) => {
              ...
              return (
                ...
                >
                  ...
                  {isDone && (
                    <span className="text-[9px] text-accent2/60 flex-shrink-0">
                      Done
                    </span>
                  )}
                </button>
        ...
        <div className="flex items-center justify-center gap-3 mb-16">
          ...
          >
            <div
              className={`w-1.5 h-1.5 rounded-full ${autoPlay ? "bg-accent2 animate-pulse" : "bg-white/20"}`}
            />
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <p className="text-white/40 text-base font-light mb-8 max-w-md mx-auto">
              Join 180,000+ creators using VIRA to turn long videos into viral
              clips automatically.
            </p>
